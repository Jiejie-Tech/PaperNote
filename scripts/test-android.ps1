...
$uiNew="$(U @(0xFF0B)) $(U @(0x65B0,0x5EFA))";$uiConfirm='OK';$uiPen=U @(0x94A2,0x7B14)
$uiUndo=U @(0x64A4,0x9500);$uiRedo=U @(0x91CD,0x505A);$uiLibrary=U @(0x8D44,0x6599,0x5E93)
$uiSettings=U @(0x8BBE,0x7F6E);$uiFinger=U @(0x5141,0x8BB8,0x624B,0x6307,0x4E66,0x5199)
$uiNotebook=U @(0x65B0,0x7B14,0x8BB0);$uiNewPage="$(U @(0xFF0B)) $(U @(0x65B0,0x9875))"
$uiPageTwo="$(U @(0x7B2C)) 2 / 2 $(U @(0x9875))";$uiMore=U @(0x66F4,0x591A)
...
Tap $uiNew;W $uiConfirm|Out-Null;Stage 'new-notebook-dialog';Tap $uiConfirm;W $uiPen|Out-Null;Stage 'editor-created';Assert-Resumed 'new notebook editor'
Enabled $uiUndo $false;$size=((A @('shell','wm','size')).Output|Out-String);if($size-notmatch '(\d+)x(\d+)'){throw 'Screen size unavailable.'}
$w=[int]$Matches[1];$h=[int]$Matches[2];$x1=[int]($w*.35);$x2=[int]($w*.68);$y1=[int]($h*.42);$y2=[int]($h*.48)
(A @('shell','input','swipe',[string]$x1,[string]$y1,[string]$x2,[string]$y2,'450')).Output|Out-Null;Start-Sleep -Milliseconds 800;Enabled $uiUndo $false;Stage 'default-finger-pan'
...
if(!$toggle){throw 'Finger toggle not found.'};TapNode $toggle;Start-Sleep -Milliseconds 500;Stage 'finger-writing-enabled'
...
Tap $uiNotebook -Contains;W $uiPen|Out-Null;Stage 'editor-reopened'
(A @('shell','input','swipe',[string]$x1,[string]$y1,[string]$x2,[string]$y2,'450')).Output|Out-Null;Start-Sleep -Milliseconds 1000;Enabled $uiUndo $true;Tap $uiUndo;Enabled $uiRedo $true;Tap $uiRedo;Stage 'finger-draw-undo-redo'
