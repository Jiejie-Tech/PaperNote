...
- 文字对象提取和页面/资料库级查找
- 随安装包提供 PP-OCRv6 Tiny 多语言模型；支持当前页图片/PDF 背景离线 OCR，以及套索选中笔迹的手写转文字
...
- Android 7.0（API 24）及以上，手机和平板响应式布局
...
- 整本批量 OCR、识别结果校对工作台和数学/LaTeX 识别引擎
- 完整标尺、角度/长度提示和更高级几何构造
- 整本批量 OCR 与校对、PDF 原文逐字选择/复制、原生文本高亮、表单、签名、测量、多个源 PDF 合并、双页阅读和分屏
- 录音设备、压缩质量设置、更精细的时间轴编辑和更强的中断恢复
- 完整无障碍、高对比度和本地插件机制
...
- 混合选区可命名保存到本机个人素材库，并在任意本机笔记中插入或删除。
- 插入后的素材仍是标准可编辑笔迹和页面对象；重复插入会生成新 ID，组关系也会安全重映射。
