...
PaperNote 现在支持 Windows 和 Android：触控笔书写、分页模板、对象与图层、PDF 文本搜索/目录/内部链接/批注列表、页面范围导出与提取、本地录音、恢复中心和带校验的备份都能离线使用。电脑与手机/平板可以直接传 `.papernote` 文件，不需要注册账号。源码和安装包已放到 GitHub / Gitee。
